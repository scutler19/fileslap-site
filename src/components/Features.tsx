...
export default function Features() {
  return (
    <section className="w-full max-w-5xl mx-auto mt-16 sm:mt-24 px-6">
      ...
      <div className="grid gap-6 sm:gap-8 md:gap-12 lg:gap-16 md:grid-cols-3">
        {features.map(({ icon, title, blurb }) => (
          <div
            key={title}
            className="flex flex-col items-center rounded-2xl bg-[#111217] p-6 sm:p-8 md:p-10 lg:p-12 text-center border border-[#1DEE7F]/10 hover:border-[#1DEE7F]/30 transition-all duration-300 hover:shadow-xl"
          >
            <div 
              className="mb-4 sm:mb-6 text-4xl sm:text-6xl drop-shadow-lg"
              aria-label={`${title} icon`}
              role="img"
            >
              {icon}
            </div>
            <h3 className="mb-3 sm:mb-4 text-lg sm:text-xl font-semibold text-[#1DEE7F]">{title}</h3>
            <p className="text-sm sm:text-base text-white/80 leading-relaxed">{blurb}</p>
          </div>
        ))}
      </div>
    </section>
