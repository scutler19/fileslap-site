export default function UseCases() {
  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-6 py-16 sm:py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
        {useCases.map((useCase, index) => (
          <div
            key={index}
            className="bg-[#111217] rounded-xl p-6 sm:p-8 border border-[#1DEE7F]/20 hover:border-[#1DEE7F]/40 transition-all duration-200 hover:shadow-lg hover:shadow-[#1DEE7F]/10"
          >
            ...
            <p className="text-sm sm:text-base text-white/70 mb-3 sm:mb-4 leading-relaxed">{useCase.description}</p>
            <div className="space-y-1 sm:space-y-2">
              {useCase.examples.map((example, exampleIndex) => (
                <div key={exampleIndex} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-[#1DEE7F] rounded-full"></div>
                  <span className="text-xs sm:text-sm text-white/60">{example}</span>
                </div>
              ))}
            </div>
          </div>
